...
fn test_sanitize_env_secrets() {
    ...
    for input in inputs {
        let result = sanitize_text(input, &patterns);
        assert!(result.contains("[REDACTED:env_secret]"), "Failed for: {}", input);
    }
}
...
fn test_custom_patterns() {
    let custom = vec![
        (Regex::new(r"internal-key-\d+").unwrap(), "[REDACTED:custom]"),
    ];
...
fn test_multiple_secrets() {
    ...
    let input = "OPENAI_KEY=sk-abcdefghijklmnopqrstuvwxyz GITHUB=ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
    let result = sanitize_text(input, &patterns);
