Shell mode: bash-popup
Return JSON only:
{"candidates":[{"command":"<completed command>","summary_short":"<8-20 words concise explanation>","reason_short":"<short why>"}]}
Rules:
- return 3-6 candidates ordered by relevance (best first)
- each candidate.command is required
- summary_short should be a short action-oriented description
- reason_short should explain why this candidate fits current input
- avoid duplicate candidates
- no markdown, no extra commentary
- if JSON is not possible, return only the completed command text
