...
function Main {
    ...
    # Setup shell integration and config files
    $shellDir = Get-ShellIntegrationFiles
    if ($shellDir) {
        # Always create/update config.default.yaml from template (will be updated on upgrades)
        Write-Info "Creating default configuration file..."
        $templateDefaultConfig = Join-Path $shellDir "config\config.default.yaml.template"

        if (Test-Path $templateDefaultConfig) {
            Copy-Item -Path $templateDefaultConfig -Destination $defaultConfigFile -Force
            Write-Success "Default configuration file created from template: $defaultConfigFile"
        } else {
            Write-Warning "Default config template not found at: $templateDefaultConfig"
            Write-Info "Creating minimal default config..."
            # Fallback: create minimal default config inline
            $defaultConfig = @"
...
"@
        ...
        if (-not (Test-Path $userConfigFile)) {
            ...
            if ($wizardConfig) {
                ...
                $templateUserConfig = Join-Path $shellDir "config\config.user.yaml.template"

                if (Test-Path $templateUserConfig) {
                    Copy-Item -Path $templateUserConfig -Destination $userConfigFile -Force
...
"@
        ...
        Write-Warning "Failed to download shell integration files"
        Write-Info "Creating minimal config files..."
